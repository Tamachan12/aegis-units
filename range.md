...
Haste (ヘイスト): all unit attack speed quickened (excluding token) if a team member.
Space-time Interference (時空干渉): speed-decrease class perk raised to 50%; all unit attack speed quickened (excluding token) if a team member.
```
...
#####Sasha (サーシャ):
```
Secret Art of Glamour (幻惑の秘術): (30s; all allies within range gain 50% physical evasion; AS: 165; WT: 35; CT: 70)
Accel Mirage (アクセルミラージュ): (30s; self: attack speed up; gives 50% physical evasion to allies within range; AS: 88; WT: 37.5; CT: 75)
```
...
Witch Attack Up (ウィッチの攻撃力アップ): witch attack +7% if Belinda is a team member.
```
...
#####Prim (プリム):
```
Prism Mirage (プリズムミラージュ): (30s; attack 1.3x; cannot be targeted by enemies; AS: ; WT: 15; CT: 30)
Prism Shadow (プリズムシャドウ): (00s; cannot be targeted by enemies; auto skill, and unlimited duration; AS: ; WT: 22.5; CT: 45)
...
Full Auto (フルオート): (18s; attack 0.9x; raises attack speed but arrow speed same; attacks 3 targets at same time; AS: 27; WT: 23.3; CT: 35)
```
...
Shadow Arrow (影矢): 20% 2-consecutive shots
...
Arbalest (アーバレスト): (25s; attack 2.7x; range 1.5x [390]; slows movement; long delay between attacks; fast shot speed; slows movement speed; AS: 102; WT: 15; CT: 30)
```
...
#####Moltena (モルテナ):
```
Hellbeast Fire Control (魔獣炎操): (20s; self and monster master token attack 2x; instant: recover +1 token; AS: 117; WT: 1; CT: 40)
Hellbeast Hellfire Control (魔獣獄炎操): (20s; self and monster master token attack 2x and range 1.5x; true damage; instant: recover +1 token ; AS: 117; WT: 5; CT: 45)
```
...
####☆Dragon Shaman Lord (ドラゴンシャーマンロード)
#####Echidna (エキドナ):
```
Exorcism Flame (破魔の炎): (15s; attack 1.8x; focus fireball attack; AS: 167; WT: 20; CT: 40)
Healing of Strong Scales (堅鱗の癒し): (30s; units affected by ability defence 1.4x; recovers HP of all allies within range; AS: 167; WT: 30; CT: 60)
```
...
Flames of Sharp Fangs (鋭牙の火炎): (25?s; units affected by ability attack 1.4x; attacks enemies with magic; AS: 167; WT: 30; CT: 60)
```

####☆Elder Bishop (エルダービショップ)
#####Celina (セレイナ):
```
Power Heal (パワーヒール): (15s; attack 2x; heals allies' HP; AS: 192; WT: 17.5; CT: 35)
Ex Power Heal (エクスパワーヒール): (20s; attack 2.5x; heals allies' HP; AS: 192; WT: 20; CT: 40)
Holy Prayer (聖なる祈り): angel cost -1 if a team member, and angel auto retreat when defeated while Celina is deployed.
```
...
####☆Elder Merchant (エルダーマーチャント)
#####Totono (トトノ):
```
Call for Help (助けを呼ぶ): (15s; focus defence; draw enemy range attacks; regenerate cost [+1/s]; AS: 0; WT: 15; CT: 30)
Desperate Call for Help (全力で助けを呼ぶ): (10s; focus defence; focus defence; draw enemy range attacks; quickly regenerate cost [+?/s]; AS: ; WT: 15; CT: 30)
Gold Get Plus (ゴールドゲットプラス): self gains 'Gold Get!', and 1.3x more gold per defeated enemy for all units with 'Gold Get!' if a team member.
```

####☆Feng Shui Devout (風水導士)
#####Krile (クルル):
```
...
Triple Play Evil Energy (重殺イービルエナジー): (15s; attack 1.3x; 3 consecutive attacks against a single target; AS: 206; WT: 17.5; CT: 35)
```
...
Magic Bullet Tathlum (魔弾タスラム): (20s*; attack 2.5x*; movement slowing effect increased; skill strengthened afterward; AS: 100; WT: 5; CT: 15*)
*2nd: 25s/2.8x/CT20; 3rd: 30s/3.2x/CT25; 4th: 35s/3.6x/CT30; 5th~: 40s/4.0x/CT35
```
...
#####Machina (マキナ):
```
Acid Bomb (アシッドボム): (30s; attack 1.8x; AoE blast 1.5x; AS: 142; WT: 1; CT: 40)
Hyper Acid Bomb (ハイパーアシッドボム): (30s; attack and AoE blast 2.5x; AS: 142; WT: 5; CT: 60)
Steam Tank (スチームタンク): possible to deploy special token that regens 50 HP/s to all allies, and decreases all enemy defence by 30% (stacks with alchemist-type attacks).
```

####☆Master Druid (マスタードルイド)
#####Laurier (ローリエ):
```
...
#####Maple (メープル):
```
Earth Grave (アースグレイブ): (30s; range 1.8x [486]; attack 1.5x; physical attack that halts movement of 3 enemies within range; AS: 200; WT: 15; CT: 30)
Earth's Wrath (大地の怒り): (30s; range 1.8x [486]; attack 3x; physical attack that halts enemy movement; AS: 200; WT: 15; CT: 30)
Guardian of Forest and Animals (森と動物の守り人): while deployed, druid, ranger, valkyrie, and bowrider attack +10%.
```
...
*2nd: 25s; 1.9x; 3rd~: 30s; 2.2x; range 1.2 [312] CT: 30
```
...
True Kamioroshi Release (真・神降ろし解除): (20s; attack 1.6x; focuses on attack until 'True Kamioroshi' used; AS: 132; WT: 5; CT: 15)
```
...
Mirage Dance (ミラージュダンス): 50% physical evasion
```
...
#####Lumaria (ルマリア):
```
Area Heal (エリアヒール): (25s; attack 1.6x; range 1.3x [273]; heals all units within range; AS: 192; WT: 12.5; CT: 25)
Wide Area Heal (ワイドエリアヒール): (25s; attack 1.3x; range 1.6x [336]; heals all units within range; AS: 192; WT: 15; CT: 30)
```

#####Sue (スー):
```
Spirit Eagle (スピリットイーグル): (30s; attack 1.9x; slows enemy movement speed; AS: 45; WT: 1; CT: 35) 
Spirit Snipe (スピリットスナイプ): (30s; attack 1.9x range 1.4x [294]; slows enemy movement speed; AS: 45; WT: 5; CT: 40)
```
