...
Arrow Rain (アローレイン): (30s; block: 0; time until attack lengthened; range 1.8x; attacks 7 enemies within range; AS: 82?; WT: 22; CT: 45)
```
...
#####Margaret (マーガレット):
```
Eternal Arrow (エターナルアロー): (00s; attack and range 1.3x [338]; unlimited duration; AS: 62; WT: 50; CT: 75)
Rainbow Arrow (レインボーアロー): (30s; block: 0; fires 7 consecutive arrows; once per battle; AS: 62; WT: 10; CT: N/A)
Clan of Radiance (輝きの一族): attack 1.2x against youkai and demons.
```
...
#####Dahlia (ダリア):
```
Abandon III (捨て身III): (10s; self: DF 0, but attack 2.6x; AS: 60; WT: 12.5; CT: 25)
Black Spear Heavy Rush (黒槍重突進): (15s; self: defenced halved; attack 2.6x and MR 2x; cost recovery 2x; self HP regeneration [30 per 0.5s]; AS: 60; WT: 15; CT: 30)
```
...
Miraculous Shield (奇跡の盾): 15% damage nullification.
```
...
Instant Death Attack (即死攻撃): all attacks have an additional 2% chance of instant death.
```
...
#####Amanda (アマンダ):
```
Abandon III (捨て身III): (10s; Self: DF 0, but attack 2.6x; AS: 42; WT: 1; CT: 25) 
Great Bandit Age (大山賊時代): (50s; attack 2x; all bandit-type class deploy cost halved and max HP x2; AS: 42; WT: 5; CT: 55)
```

#####Conrad (コンラッド):
```
Abandon III (捨て身III): (10s; Self: DF 0, but attack 2.6x; AS: 83; WT: 16.7 ; CT: 25) 
...
Full Swing (フルスイング): (20s; attack 2.7x; self: defence 0; attacks all nearby land enemies within AoE; AS: 83; WT: 17.5; CT: 35)
```
...
Spike Shield (スパイクシールド): (40s; defence 2x; focus defence; counter* blocked enemies' physical attacks; AS: 0; WT: 26.7; CT: 40)
*1/2 of enemy attack stat returned as true damage.
```
...
Earthquake Attack (大震撃): (30s; attack 3x; defence 2x; attacks all nearby land enemies; attack speed slow;  AS: 144; WT: 5; CT: 45)
```
...
New Holy Awakening (新聖なる覚醒): (70s; attack and defence 2.5x; AS: 52; WT: 7.5; CT: None)
Miraculous Shield (奇跡の盾): 15% damage nullification.
```
...
Compliant Rod 'Stone Monkey' (如意棒・石猿): (15s; becomes 1 block melee; attack 2.5x; defence 1.5x; AS: 78; WT: 22.5; CT: 45)
```

####☆Great Spirit of Battle (戦の大聖霊)
#####Cynthia (シンシア):
```
...
#####Sarah (セーラ):
```
Princess Guard Ceremonial Halberd Art 'Defence Type' (護姫式槍斧術・守の型): (40s; self, Prince, and princess-type defence 1.5x; AS: 66; WT: 1; CT: 60)
Princess Guard Ceremonial Halberd Art 'Attack/Defence Type' (宮廷槍斧術・攻守の型): (30s; self, Prince and princess-type attack and defence 1.4x; self: physical evade 90%?; AS: 66; WT: 5; CT: 60)
```
...
#####Lilia (リリア):
```
Divine Sword Excalibur (神剣エクスカリバー): (40s; defence 2.4x; status ailment immunity; AS: 67; WT: 27.5; CT: 55)
Divine Sword Caliburn (神剣キャリバーン): (60s; attack and defence 2x; MR 2x; self: status ailment immunity; AS: 67; WT: 35; CT: 70)
```
...
#####Sherry (シェリー):
```
Magic Sword Arondight (魔剣アロンダイト): (40s; attack 2x; AS: 67; WT: 27.5; CT: 55)
Holy Sword Arondight (聖剣アロンダイト): (15s; block: 0; focus defence; skill changes afterwards; AS: ; WT: 5; CT: 10)
Holy Sword Arondight (聖剣アロンダイト): (25s; attack 1.7x; range [250] attack; attacks up to 5 enemies within range; skill changes afterwards; AS: 67; WT: ; CT: 30)
```
...
#####Sybilla (シビラ):
```
Magic Sword Fragarach (魔剣フラガラッハ): (40s; 1.7x attack; instant death attack [10%]; AS: 67; WT: 1; CT: 55) 
Magic Sword Answerer (魔剣アンサラー): (40s; 1.9x attack; range attack increased [280]; AS: 67; WT: 5; CT: 55)
```

#####Themis (テミス):
```
Treasured Sword Galatine (宝剣ガラティン): (60s; defence 2x; AS: 74; WT: 36.7; CT: 55)
Strong Sword Galatine (剛剣ガラティン): (25s; attack 2.3x; AS: 74; WT: 30; CT: 45)
```

####☆Immortal Queen (イモータルクイーン)
#####Karma (カルマ):
```
Divine Sword Tyrfing (神剣ティルフィング): (13s; attack and defence 1.3x; self: HP won't drop to 0; AS: 79; WT: 1; CT: 75) 
True Divine Sword Tyrfing (真神剣ティルフィング): (15s; attack 1.6x; self: defence 0, but HP won't drop to 0; AS: 79; WT:  5; CT: 65)
```
...
#####Berna (ベルナ):
```
Assassination III (暗殺III): (25s; 1.8x attack; instant death attack [33%]; AS: 43; WT: 1; CT: 40) 
Whirlwind of Death (死の旋風): (30s; 1.9x attack; attack all nearby enemies; instant death attack [33%]; AS: 43; WT: 5; CT: 45)
```

#####Betty (ベティ):
```
Assassination I (暗殺I): (15s; 1.5x attack; instant death attack [10%]; AS: 43; WT: 33; CT: 50)
Super Assassination (超暗殺): (15s; 2x attack; instant death attack [30%]; AS: 43; WT: 16.7; CT: 25)
```
...
Advanced Notification of Death (死の予告状): (30s; 1.6x attack; ranged attack; instant death attack [10%]; AS: 43; WT: 33.3; CT: 50)
```

#####Junon (ジュノン):
```
Target Vitals (急所狙い): (45s; attack 0.9x; assassination [10%]; AS: 43; WT: 10; CT: 20)
Assassin Dagger (凶刃): (20s; attack 1.8x; assassination [10%]; skill automatically activates; AS: 43; WT: 5; CT: 10)
```
...
#####Esther (エスタ):
```
Pegasus Wing (ペガサスウイング): (45s; flight; attacks all nearby targets; AS: 62; WT: 1; CT: 25) 
Heavenly Wings of Destruction (撃滅の天翼): (60s; self: defence -20%; 1.5x attack; flight; attacks everything nearby in a wider area; AS: 62; WT:  5; CT: 35)
```

#####Isabelle (イザベル):
```
Icarus Wing (イカロスウィング): (35s; attack 1.3x*; flight; attacks all nearby targets; skill weakened afterwards; can use 3 times per battle; AS: 62; WT: 12.5; CT: 20)
*2nd: 1.2x; 3rd: 1.1x
...
#####Alicia (アリシア):
```
Strengthen Defence IV (防御力強化IV): (45s; 2.0x defence; AS: 74; WT: 12.5; CT: 25) 
Giant Killing (ジャイアントキリング): (50s; attack speed slow; attack and defence 3x; AS: 86; WT: 50; CT: 100)
Spear of Victory (勝利の槍): Royal Guard, Valkyrie, Dragon Rider, and Pegasus Rider attack and defence +5% if Alicia is a team member.
```
...
#####Alice (アリス):
```
Hard Qigong (硬気功): (60s; defence 2.0x; block #+1; counter blocked enemies' physical attacks; AS: 53; WT: 1; CT: 75)
Riki Kattangong (剛力活丹功): (30s; attack 2.0x*, Max HP permanently raised by 1.1x*; skill strengthened afterwards; AS: 53; WT: 5; CT: 30)
*2nd: attack 2.2*, HP 1.4x; 3rd: attack 2.5x, HP 1.7x; 4th~: attack 2.5x
```

#####Gina (ジーナ):
```
Rush! (ラッシュ！): (20s; attack speed faster; AS: 31; WT: 20; CT: 30) 
Pump-Up (パンプアップ): (60s; 3.0x HP; 1.5x attack; AS: 53; WT:  56.7; CT: 85)
```
...
#####Momo (モモ):
```
Strengthen Attack III (攻撃力強化III): (35s; attack 1.8x; AS: 53; WT: 16.7; CT: 25) 
Hageki Fist (破撃の拳): (25s; 2.3x attack; skill automatic; skill changes after effect ends; AS: 53; WT: 6.7; CT: 10)
Goshin Fist (護身の拳): (25s; 2.3x defence; skill automatic; skill changes after effect ends; AS: 53; WT: N/A; CT: 10)
```

#####Ur (ウル):
```
Rush! (ラッシュ！): (20s; attack speed faster; AS: 31; WT: 15; CT: 30) 
Spinning Claw (スピニングクロー): (35s; attack and HP 2.2x; true damage; skill weakened afterwards; AS: 53; WT: 15; CT: 30)
*2nd: 1.6x; 3rd~: 1.2x
```
...
#####Amélie (航海士アメリ):
```
Stealth Shot (ステルスショット): (55s; block 0 range [200] attack; cannot be targeted by enemy ranged attacks; AS: 75; WT: 15; CT: 30)
Stealthful Shot (ステルスフルショット): (30s; attack 1.6x; block 0 range [200] attack; cannot be targeted by enemy ranged attacks; AS: 75; WT: 15; CT: 30)
First Mate's Command (一等航海士の指揮): while deployed, sailor and pirate attack +10%.
```
...
Power Attack (パワーアタック): attack power up, but attack speed slower.
```
...
#####Jake (ジェイク):
```
Guardian's Aegis (守護者の神盾): (20s; defence 2.0x and block+1; focus defence; counters physical attacks; cost regen up [+1/s]; AS: 0; WT: 20; CT: 40)
Inviolable Aegis (不可侵の神盾): (20s; unable to counter; defence and MR 2x; block+1; cost regen up [+1/s]; AS: 0; WT: 17.5; CT: 35)
Magic Resistance (魔法耐性): +10% magic resistance
```
...
Limiter Cut (リミッターカット): (30s; attack and defence 2.3x; current HP halved when skill ends; range [180]; AS: 102; WT: 5; CT: 50)
```
...
#####Palffy (パルフィ):
```
Maiden's Provocation (乙女の挑発): (30s; attack and defence 1.5x; draws ranged enemy focus; AS: 102; WT: 20; CT: 40) 
Iron Wall Provocation (鉄壁の挑発): (60s; defence 2.2x; magic resistance 2x; draws ranged enemy focus; AS: 102; WT: 30; CT: 60) 
```
...
Firepower Support Command (火力支援指揮): while deployed, pirate, gunner, and merchant class attack +10%.
```

#####Matsuri (マツリ):
```
Devote Heart and Soul (一意専心): (80s; block and attack # +1; attack and defence 1.4x; AS: 32; WT: 1; CT: 95) 
Lightning Speed (疾風迅雷): (40s; attack 1.8x; block: 1; range attack [280]; slows enemy movement; AS: 32; WT: 5; CT: 50)
```
...
#####Racua (ラキュア):
```
True Ancestor's Bloodline (真祖の血脈): (20s; attack 1.5x; decrease all damage Racua receives, except magic, by 50%; AS: 72; WT: 25; CT: 50)
Near Immortal (ニアイモータル): (20s; attack 2x; decreases all damage except magic by 70%; AS: 72; WT: 27; CT: 55)
```
